...
class AuthController extends BaseController {
	...
	public function postSignin()
	{
		...
		try
		{
			...
			Sentry::authenticate(Input::only('email', 'password'), Input::get('remember-me', 1));
	...
	public function postSignup()
	{
		...
		try
		{
			...
			Mail::send('emails.register-activate', $data, function($m) use ($user)
			{
				...
				$m->subject('B ' . $user->first_name);
			});
*/
			// Redirect to the register page
		//	return Redirect::back()->with('success', Lang::get('auth/message.signup.success'));
		
					Sentry::authenticate(array("email"=>Input::get('email'),"password"=>Input::get('password')), 1);
				// Get the page we were before
			$redirect = Session::get('loginRedirect', 'checkout');
